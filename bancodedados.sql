...
--altera dados na id 3
update usuarios set nome = 'Raja W. Coffey Thomas' where id = 3;

--deleta linhas com e-mail duplicados
delete from usuarios where email = 'raja.feugiat@nonummy.com';

--impede criação de e-mails duplicados
alter table usuarios add constraint email_unique_usuarios unique(email);

--insere usuário cadastrado com e-mail duplicado
insert into usuarios (nome, idade, email, senha) values ('Raja W. Coffey Thomas', 30, 'raja.feugiat@nonummy.com', 's5F51T7L');
...
--altera a situação do usuario cadastrado no e-mail informado para falso
...
--exclui coluna idade e add coluna data_nascimento
alter table usuarios drop column idade, add column data_nascimento date;

--altera a senha do usuário cadastrado no email informado
